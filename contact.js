...
window.addEventListener('resize', () => {
  ...
  function icon(x, ind) {
    x.innerHTML = iconArray[ind % iconArray.length]
  }

  function text(x, ind) {
    x.innerHTML = textArray[ind % textArray.length]
  }
  ...
  if (windowWidth <= 768) {
    ...
    links.forEach((e, index) => {
      ...
      contact_text.style.fontSize = `45px`;
      contact_second_text.style.fontSize  =`25px`
      contact_text.style.fontWeight = 'lighter'
    });
  } else {
    links.forEach((e, index) => {
